...
##                self.serialport.set(GPIB_USB_ADAPTER_ADRESS)
                ...
                #self.color = ['red', '#ff9900', 'blue', '#00cc00', 'black']
		...
		############### SECOND_FEEDBACK
		 ### self.chillerFeedback_LF
                ...
                self.T_set_b.set(0)
                self.T_min_b = IntVar()
                self.T_min_b.set(211)
                self.T_max_b = IntVar()
                self.T_max_b.set(233)
                self.Vpi_lim_low_b = DoubleVar()
                self.Vpi_lim_low_b.set(0.3*PIEZO_VOLTAGE_MAX_b)
                self.Vpi_lim_high_b = DoubleVar()
                self.Vpi_lim_high_b.set(0.7*PIEZO_VOLTAGE_MAX_b)
                self.T_delta_t_b = DoubleVar()
                self.T_delta_t_b.set(30)
                self.T_feedback_b = IntVar()
                self.T_feedback_b.set(0)
                
                self.counter_T_feedback_b = 0.
                self.counter_T_feedback_reset_bool_b = 1
                
                Label(self.chillerFeedback_LF_b, text = "T_set").pack(side = LEFT)
                self.T_setE_b= Entry(self.chillerFeedback_LF_b, textvariable=self.T_set_b, bg='white', width = 5)
                self.T_setE_b.pack(side = LEFT)

                Label(self.chillerFeedback_LF_b, text = "T_min").pack(side = LEFT)
                self.T_minE_b= Entry(self.chillerFeedback_LF_b, textvariable=self.T_min_b, bg='white', width = 5)
                self.T_minE_b.pack(side = LEFT)

                Label(self.chillerFeedback_LF_b, text = "T_max").pack(side = LEFT)
                self.T_maxE_b= Entry(self.chillerFeedback_LF_b, textvariable=self.T_max_b, bg='white', width = 5)
                self.T_maxE_b.pack(side = LEFT)

                Label(self.chillerFeedback_LF_b, text = "Vpi_lim_low").pack(side = LEFT)
                self.Vpi_lim_lowE_b= Entry(self.chillerFeedback_LF_b, textvariable=self.Vpi_lim_low_b, bg='white', width = 5)
                self.Vpi_lim_lowE_b.pack(side = LEFT)

                Label(self.chillerFeedback_LF_b, text = "Vpi_lim_high").pack(side = LEFT)
                self.Vpi_lim_highE_b= Entry(self.chillerFeedback_LF_b, textvariable=self.Vpi_lim_high_b, bg='white', width = 5)
                self.Vpi_lim_highE_b.pack(side = LEFT)

                Label(self.chillerFeedback_LF_b, text = "T_delta_t").pack(side = LEFT)
                self.T_delta_tE_b= Entry(self.chillerFeedback_LF_b, textvariable=self.T_delta_t_b, bg='white', width = 5)
                self.T_delta_tE_b.pack(side = LEFT)
                
                self.T_feedbackCB_b = Checkbutton(self.chillerFeedback_LF_b, text = 'lock', variable = self.T_feedback_b)
                self.T_feedbackCB_b.pack(side = LEFT, padx = 10)
        ...
        def initSerialToChiller_and_NI_daqmx(self):
                ...
                while attemps_connect_to_chiller < 3:
                        try :
                                self.ser = serialChiller.open_chiller_port('/dev/ttyUSB0')
...
##                                print 'set temp.',settemp
		...
		attemps_connect_to_chiller = 0
                while attemps_connect_to_chiller < 3:
                        try:
                                self.ser_b = serialChiller.open_chiller_port(3)
                                print 'COMB: establised connection with the ', attemps_connect_to_chiller, ' attemp' 
##                                status = readChillerStatus(serialPort = self.ser)
                                realtemp_b = serialChiller.readCoolantTemperature(serialPort = self.ser_b)
##                                settemp = readSetTemperature(serialPort = self.ser)
####                                print 'status',status
                                print 'COMB: actual temp',realtemp_b
##                                print 'set temp.',settemp
                                ...
                                attemps_connect_to_chiller += 1
                                time.sleep(0.5)
                                print 'COMB: not attemp', attemps_connect_to_chiller
                                pass
...
##                print 'set temp.',settemp
		...
		self.data = init_channel(self.taskHandle,self.nr_samples,10000.0)
                self.data_b = init_channel(self.taskHandle_b,self.nr_samples,10000.0, channel = 'Dev/ai1')
...
##                                                                print 'WARNING - could not manually READ the set temperature at t = ',self.t[-1]
        ...
        def feedback_T_b(self):
                #xxx
                #print 'start'
                if self.counter_T_feedback_reset_bool_b == 1:
                        self.counter_T_feedback_b = self.t[-1]
                        self.counter_T_feedback_reset_bool_b = 0
                else:
                        if (self.t[-1] - self.counter_T_feedback_b) > self.T_delta_t_b.get(): # so every self.T_delta_t seconds
                                
                                self.counter_T_feedback_reset_bool_b = 1
                                
                                try:
                                        last_piezo_voltage_b = np.mean(self.dat[1][-5:]) # tries to average over the last 5 temperature values if possible
                                except:
                                        last_piezo_voltage_b = self.dat[1][-1] # takes last value of data array of the index corresponding to the name piezo_voltage
                                
                                if last_piezo_voltage_b < 0.05:
                                        print 'COMB: last piezo voltage was smaller 0.05 V, piezo feedback close?'
                                else:
                                
                                        change_temperature_b = 0
                                        
                                        if last_piezo_voltage_b > self.Vpi_lim_high_b.get():
                                                new_set_temp_b = self.T_set_b.get() - 1
                                                if new_set_temp_b > self.T_min_b.get():
                                                        self.T_set_b.set(new_set_temp_b)
                                                        change_temperature_b = 1
                                                else:
                                                        print 'COMB: Temp feedback reached higher specified limit'
                                        
                                        if last_piezo_voltage_b < self.Vpi_lim_low_b.get():
                                                new_set_temp_b = self.T_set_b.get() + 1
                                                if new_set_temp_b < self.T_max_b.get():
                                                        self.T_set_b.set(new_set_temp_b)
                                                        change_temperature_b = 1
                                                else:
                                                        print 'COMB: Temp feedback reached lower specified limit'
                                                
                                        if change_temperature_b == 1:
                                                time.sleep(0.3)
                                                try:
                                                        changedTemperature_b = serialChiller_b.setTemperature(serialPort = self.ser, temp = self.T_set_b.get())
                                                        print 'COMB: CHANGED baseplate temperature at t = ',round(self.t[-1]), ' s  to  ', new_set_temp_b
                                                except:
                                                        print 'COMB: WARNING - could not CHANGE the set temperature at t = ',self.t[-1]
                                                
                                                if changedTemperature_b != -1 and changedTemperature_b > self.T_min_b.get() and changedTemperature < self.T_max.get():
                                                        self.T_set_b.set(changedTemperature_b)
                                                else: # in case just the reply from the changed temperature hanged, we ask for the temperature one more time manually
##                                                        try:
##                                                                changedTemperature = readSetTemperature(self.ser)
##                                                        except:
##                                                                print 'WARNING - could not manually READ the set temperature at t = ',self.t[-1]
                                                        if changedTemperature_b != -1 and changedTemperature_b > self.T_min_b.get() and changedTemperature_b < self.T_max_b.get() :
                                                                self.T_set_b.set(changedTemperature_b)
                                                                print 'COMB: determined changed baseplate by manually asking, instead of the reply from serial_Chiller255p.setTemperature' 
                                                ...
                                                #print 'tried to change chiller temperatur'
                                        print 'COMB: checked baseplate temperature at t = ',round(self.t[-1]), ' s'                           
